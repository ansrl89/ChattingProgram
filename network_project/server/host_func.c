...
void analyze_Message() {
	...
	if(strcasecmp(command, "@join") == 0){
		strcpy(clntPort, strtok(NULL, "/"));
	}
	if(strcasecmp(command, "@list") == 0){
		strcpy(purpose, strtok(NULL, "/"));
	}
}
...
void handle_S_JOIN() {
	...
	strcpy(last->nextLink->ip, inet_ntoa(sock_New_addr.sin_addr));
	strcpy(last->nextLink->port, clntPort);
...
void handle_S_LIST() {
	...
	while (obj->nextLink != NULL ) {
		char msg[50] = { 0, };
		sprintf(msg, "%s %s %s ", obj->nextLink->nick, obj->nextLink->ip,
				obj->nextLink->port);
